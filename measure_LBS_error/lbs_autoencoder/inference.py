...
from measure_LBS_error.lbs_autoencoder.model import LBSModel
from measure_LBS_error.lbs_autoencoder.utils import flatten_lbs


model_path = "/home/junmyeong/workspace/JM_LBSRecon/measure_LBS_error/lbs_autoencoder/best.tar"

lbs_autoencoder = LBSModel().cuda()
lbs_autoencoder.load_state_dict(torch.load(model_path)['state_dict'])

def compress_lbs(lbs : torch.tensor):    
    # inference lbs encoder
    with torch.no_grad():
        pred_lbs = lbs_autoencoder.encoder(lbs)
    
    return pred_lbs

def decompress_lbs(compressed_lbs : torch.tensor):
    with torch.no_grad():
        pred_lbs = lbs_autoencoder.decoder(compressed_lbs)
...
def main():
    ...
    smpl_model = smplx.create(model_path=os.path.join(resource_path, "/home/junmyeong/workspace/JM_LBSRecon/measure_LBS_error/resource/smpl_models"),
                                    model_type='smplx',
                                    gender='male',
